...
4. **RAD-PROTOCOLO (ADR-0112 — central autônoma)** — número de protocolo de chamada. Baixa do `rdebruem/rad-ecosystem` o AGI Python (`/var/lib/asterisk/agi-bin/`) e o stub de dialplan (`/etc/asterisk/extensions_rad.conf`), e semeia um padrão default em `/etc/rad-pbx/protocol-pattern.json`. A central é **independente da Platform**: o AGI lê o padrão do arquivo local a cada chamada e grava o protocolo no `CDR(accountcode)` — sem rede, sem sync. A Platform sobrescreve o arquivo de padrão **via SSH** ao salvar na UI, e lê os registros **do CDR**. **Não altera roteamento**: o contexto `[rad-protocolo]` fica inerte até a Inbound Route fazer `Gosub(rad-protocolo,s,1)` (wiring manual e validado — ver runbook `protocol-cutover` no vault). Requer `python3` (o de fábrica do CentOS 7, 3.6.8, basta).
...
- Para a opção 4 (RAD-PROTOCOLO): `python3` instalado (CentOS 7: `yum install -y python3`) e os scripts do RAD-PROTOCOLO presentes na `main` do `rad-ecosystem`. (Não requer mais `systemd` — o modelo autônomo do ADR-0112 não usa o serviço de sync.)
