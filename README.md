...
6. **OpenVPN Client (conexão da central ao DC RAD)** — instala o cliente OpenVPN no servidor Issabel e conecta-o à VPN do datacenter de controle da RAD (onde vive a Platform). Baixa do `rdebruem/rad-ecosystem` o template padrão em `apps/rad-pbx-platform/scripts/openvpn-client/setup-default.sh` (a senha VPN é compartilhada do grupo — vive no repo privado). Antes de rodar, o admin precisa exportar o perfil VPN do pfSense (**OpenVPN Client Export → Inline Configurations: Archive**) e enviar o `.zip` resultante pro servidor via `scp`. A opção pergunta o **slug do cliente** (lowercase + dígitos + hífen, ex: `acme`) e o **path do .zip**, e o template então: instala `openvpn` + `unzip` (via yum/dnf + EPEL), extrai o zip em `/etc/openvpn/client/`, cria o arquivo `auth` com `voip.${slug}` + senha do grupo (mode 600 root:root), renomeia o `.ovpn` para `${slug}.conf`, ajusta `auth-user-pass auth` e remove as linhas `data-ciphers` (incompatibilidade com cipher legado do servidor OpenVPN do DC), e sobe `openvpn-client@${slug}.service` via systemd. Aguarda 5s e valida que a unit ficou ativa + IP na interface `tun*`; em falha, despeja `journalctl -n 30`.
...
- Para a opção 6 (OpenVPN Client): `.zip` do perfil VPN exportado do pfSense (**OpenVPN Client Export → Inline Configurations: Archive**) já enviado pro servidor via `scp` antes de rodar. EPEL alcançável (o instalador habilita se ausente). O cliente OpenVPN e `unzip` são instalados pelo template (não precisa instalar antes).
